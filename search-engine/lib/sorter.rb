class Sorter
  ...
  SORT_ORDERS = ["default", "ci", "cmc", "color", "name", "new", "newall", "number", "old", "oldall", "pow", "power", "rand", "random", "rarity", "tou", "toughness", "artist", "released", "set", "firstprint", "lastprint", "mv", "usd", "usd_printing"].sort
  ...
  def card_key(c)
    @sort_order.flat_map do |part|
      ...
      when "-lastprint"
        [c.last_release_date.to_i_sort]
      when "usd_printing"
        [c.prices ? 0 : 1, c.prices ? -c.prices["usd"].to_f : 0]
      when "-usd_printing"
        [c.prices ? 0 : 1, c.prices ? c.prices["usd"].to_f : 0]
      when "usd"
